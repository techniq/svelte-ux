...
<script lang="ts">
  ...
  interface Props {
    name?: any;
    value?: any; // index or value
    autoscroll?: boolean;
    variant?: 'default' | 'outline' | 'fill' | 'fill-light' | 'fill-surface' | 'underline' | 'none';
    size?: 'xs' | 'sm' | 'md' | 'lg';
  ...
  const selectedOption = writable<HTMLElement | undefined>();
  const selectedPanel = writable<HTMLElement | undefined>();
...
<div class={cls('ToggleGroup', `variant-${variant}`, $classesStore.root, className)} {...restProps}>
